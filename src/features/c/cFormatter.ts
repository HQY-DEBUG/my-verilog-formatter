...
import { ClangLineRange, runClangFormat } from './clangFormat';
...
export function formatC(code: string, range?: ClangLineRange): string {
...
function protectCLayoutRegions(code: string): { code: string; blocks: Map<string, string> } {
    ...
    while (code.includes(markerPrefix)) { markerPrefix += '_'; }
    ...
    for (const match of code.matchAll(C_LITERALS_AND_COMMENTS)) {
        ...
        if (endLine > tokenLine) {
            for (let line = tokenLine; line <= endLine; line++) { protectedLines.add(line); }
        }
        if (/^(?:\/\/|\/\*)\s*clang-format off\b/.test(match[0])) {
            disabledStart ??= tokenLine;
        } else if (/^(?:\/\/|\/\*)\s*clang-format on\b/.test(match[0]) && disabledStart !== undefined) {
            for (let line = disabledStart; line <= endLine; line++) { protectedLines.add(line); }
    ...
    if (disabledStart !== undefined) {
        for (let line = disabledStart; line < lines.length; line++) { protectedLines.add(line); }
    }
    for (let i = 0; i < lines.length; i++) {
        if (!/^\s*#/.test(maskedLines[i]) || !lines[i].trimEnd().endsWith('\\')) { continue; }
        do {
...
function parseAssignment(line: string, maskedLine: string): AssignmentLine | undefined {
...
function alignEnumDeclarations(code: string): string {
    const lines = code.split('\n');
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (openingLine >= lines.length) { break; }
...
function alignEnumMemberRange(lines: string[], start: number, end: number): void {
    for (let i = start; i < end;) {
        ...
        const members: EnumMemberLine[] = [first];
        let blockEnd = i + 1;
        while (blockEnd < end) {
...
function parseEnumMember(line: string): EnumMemberLine | undefined {
    ...
    if (comma) { code = code.slice(0, -1).trimEnd(); }
    ...
    if (equal >= 0 && !value) { return undefined; }
    return {
        indent,
        name,
...
export class CFormatter implements
    ...
    constructor(private readonly clang = runClangFormat) {}
    ...
    private async formatDocument(document: vscode.TextDocument, range?: ClangLineRange): Promise<vscode.TextEdit[]> {
        ...
        if (formatted === original || document.version !== version) { return []; }
...
function affectedLineRange(original: string, formatted: string, range: ClangLineRange): ClangLineRange {
    const before = original.split(/\r?\n/);
    const after = formatted.split(/\r?\n/);
    let start = 0;
    while (start < range.start - 1 && before[start] === after[start]) { start++; }
    let suffix = 0;
