...
exports.MatlabFormatter = void 0;
exports.formatMatlab = formatMatlab;
...
function reindentMatlabBlocks(code) {
    const result = [];
    const blocks = [];
    let depth = 0;
...
function alignMatlabAssignments(code) {
    ...
    for (let i = 0; i < lines.length;) {
        ...
        if (block.length === 1) {
            ...
            const statements = block.map(item => (`${item.name.padEnd(maxName)} = ${item.value.padEnd(maxValue)}${item.semicolon ? ';' : ''}`));
...
function isMatlabConfigurationSeparator(line) {
    const trimmed = line.trim();
    if (!trimmed) {
        return true;
    }
...
function formatMatlabAssignment(item) {
    const comment = item.comment ? ` ${item.comment}` : '';
    return `${item.indent}${item.name} = ${item.value}${item.semicolon ? ';' : ''}${comment}`;
}
function normalizeMatlabExecutionAssignment(line) {
    ...
    if (!match || isMatlabConfigurationLiteral(match[3])) {
        return undefined;
    }
...
function parseMatlabAssignment(line) {
    const commentIndex = findMatlabCommentIndex(line);
    const comment = commentIndex >= 0 ? line.slice(commentIndex).trim() : '';
    const statement = (commentIndex >= 0 ? line.slice(0, commentIndex) : line).trimEnd();
    const match = statement.match(/^(\s*)([A-Za-z]\w*(?:\s*\.\s*[A-Za-z]\w*)*)\s*=\s*(?!=)(.*?)\s*$/);
    if (!match || !match[3] || !isMatlabConfigurationLiteral(match[3])) {
        return undefined;
    }
...
function normalizeMatlabValue(value) {
    return value.trimEnd().replace(/\s*;$/, ';');
}
function isMatlabConfigurationLiteral(value) {
...
function findMatlabCommentIndex(line) {
    let quote = '';
...
//# sourceMappingURL=matlabFormatter.js.map