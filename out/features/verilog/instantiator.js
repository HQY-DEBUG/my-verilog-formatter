...
exports.registerInstantiatorCommands = registerInstantiatorCommands;
 ...
 * @param context 扩展上下文
 */
function registerInstantiatorCommands(context) {
    // 一键例化
    context.subscriptions.push(vscode.commands.registerCommand('verilogFormatter.instantiate', async () => {
    ...
    // 一键 TB
    context.subscriptions.push(vscode.commands.registerCommand('verilogFormatter.generateTb', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
