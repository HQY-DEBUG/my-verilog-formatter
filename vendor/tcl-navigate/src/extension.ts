...
class TclDocumentSymbolProvider implements vscode.DocumentSymbolProvider {
	...
	private processNamespaces(
		...
		while ((nsMatch = nsRegex.exec(text)) !== null) {
			...
			const nsSymbol = new vscode.DocumentSymbol(
				...
				range,
				new vscode.Range(startPos, endPos.isBefore(document.lineAt(startPos.line).range.end) ? endPos : document.lineAt(startPos.line).range.end)
			);
	...
	private processProcedures(
		...
		while ((procMatch = procRegex.exec(text)) !== null) {
			...
			// 2026/09/21 修改：单行过程后有其他命令时，名称选区不能超出符号范围。
			const range = new vscode.Range(startPos, endPos);
			const selectionRange = new vscode.Range(startPos, endPos.isBefore(document.lineAt(startPos.line).range.end) ? endPos : document.lineAt(startPos.line).range.end);
	...
	private findMatchingBrace(text: string, openBracePos: number): number | null {
		// 2026/09/21 修改：校验起点并跳过转义字符，正确处理 Tcl 的转义花括号。
		if (openBracePos < 0 || text[openBracePos] !== '{') return null;
		let braceCount = 1;
...
class TclDefinitionProvider implements vscode.DefinitionProvider {
	...
	private isVariableReference(lineText: string, charPos: number): boolean {
        // 2026/09/21 修改：只检查当前词语前的变量前缀，不跨空白误判其他命令。
        let index = charPos;
        while (index > 0 && /[\w:]/.test(lineText[index - 1])) index--;
        return lineText[index - 1] === '$' || (lineText[index - 1] === '{' && lineText[index - 2] === '$');
    }
	...
	private async findNamespaceQualifiedDefinition(
        ...
        // 2026/09/21 修改：先查当前缓冲区的限定名定义，工作区扫描时禁止递归回查。
        const direct = await this.findProcedureDefinition(document, qualifiedName);
		...
		return searchWorkspace ? this.searchInWorkspace(qualifiedName) : null;
	}
	 ...
	 */
    // 2026/09/21 修改：保留命名空间分隔符，工作区查找复用当前文档解析，避免错误拼接名称。
    private async searchInWorkspace(symbolName: string): Promise<vscode.Location | null> {
        ...
        const exclusions = Object.keys(searchExclude).filter(key => searchExclude[key]);
        const files = await vscode.workspace.findFiles('**/*.tcl', exclusions.length ? '{' + exclusions.join(',') + '}' : undefined);
        for (const file of files) {
            const doc = await vscode.workspace.openTextDocument(file);
            const location = symbolName.includes('::')
                ? await this.findNamespaceQualifiedDefinition(doc, symbolName, false)
                : await this.findProcedureDefinition(doc, symbolName) || this.findNamespaceDefinition(doc, symbolName);
            if (location) return location;
            if (!symbolName.includes('::')) {
                const qualified = await this.findProcedureDefinition(doc, '[\\w:]+::' + symbolName);
...
class TclHoverProvider implements vscode.HoverProvider {
	...
	private extractDocComment(text: string, symbolPos: number): string {
		...
		while (currentPos >= 0) {
			...
			// 2026/09/21 修改：文件首行注释也属于过程文档，处理完成后再退出。
...
function configureTclLanguage() {
	// 2026/09/21 修改：将运行期语言配置纳入宿主生命周期。
	return vscode.languages.setLanguageConfiguration('tcl', {
		indentationRules: {
...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(configureTclLanguage());
	...
	// 2026/09/21 修改：作为内置组件静默启动，避免每次打开工作区弹出通知。
}
