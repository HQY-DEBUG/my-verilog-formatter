...
async function run_rbql_query(webview, input_path, csv_encoding, backend_language, rbql_query, output_dialect, with_headers, integration_test_options=null) {
    // 2026/09/21 新增：RBQL 可执行 JavaScript/Python，仅在受信任工作区运行。
    if (!vscode.workspace.isTrusted) {
        vscode.window.showWarningMessage('RBQL 查询需要受信任的工作区。');
        return;
    }
...
function deactivate() {
    // 2026/09/21 修改：内置组件退出时释放动态注册的资源和延时任务。
    for (const disposable of [
        rbql_preview_panel, dialect_panel, debug_log_output_channel,
        extension_context.lint_status_bar_button, rbql_status_bar_button, align_shrink_button,
        copy_back_button, column_info_button, dynamic_dialect_select_button,
        doc_first_edit_subscription, column_info_cursor_subscription, alt_background_cursor_subscription,
        rainbow_token_event, comment_token_event, row_background_decoration_event,
        column_tracking_decoration_event, sticky_header_disposable, inlay_hint_disposable,
        alternate_row_background_decoration_type, ...tracked_field_decoration_types,
    ]) {
        if (disposable) disposable.dispose();
    }
    if (cursor_timeout_handle_for_column_info) clearTimeout(cursor_timeout_handle_for_column_info);
    if (cursor_timeout_handle_for_alt_row_background) clearTimeout(cursor_timeout_handle_for_alt_row_background);
}
